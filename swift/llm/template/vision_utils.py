...
def _build_normalize(input_size):
    MEAN, STD = IMAGENET_MEAN, IMAGENET_STD
    transform = T.Compose([
        T.ToTensor(),
        T.Normalize(mean=MEAN, std=STD)
...
def _build_transform_aug(input_size):
    trans = []
    # choices = ['resize_then_crop', 'random_flip', 'random_rotate','blur','brightness_contrast','colorjitter','compression']
    choices = ['resize', 'random_flip', 'random_rotate','blur','brightness_contrast','colorjitter','compression']
    if 'resize' in choices:
        trans += [A.Resize(height=input_size, width=input_size, interpolation=cv2.INTER_CUBIC)]
    if 'random_crop' in choices:
        trans += [A.RandomCrop(height=input_size, width=input_size, p=1)]
    if 'random_resize_crop' in choices:
        trans += [A.RandomResizedCrop(size=input_size, scale=(0.7, 1.0), ratio=(0.75, 1.333), p=1)]
    ...
    if 'random_rotate' in choices:
        trans += [A.Rotate(limit=[-10, 10], p=0.5)]
    if 'colorjitter' in choices:
        trans += [A.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.2, p=0.5)]
    if 'blur' in choices:
        trans += [A.GaussianBlur(blur_limit=[3, 7], p=0.5)]
    ...
    if 'compression' in choices:
        trans += [A.ImageCompression(quality_range=[40, 100], p=0.5)]
    if 'random_erasing' in choices:
        # scale: erasing面积的范围  ratio: erasing长宽比的范围
        trans += [A.Erasing(scale=(0.1, 0.4), ratio=(0.5, 2.0), fill_value=0, p=0.5)]
...
def transform_image(image, input_size=448, max_num=12, aug=False, pre_resize=False, aug_lq_only=False, aug_hq_only=False, pre_aug=False, gaussian_sigma=None, jpeg_quality=None, resize_robust=None):
    images = _dynamic_preprocess(image, image_size=input_size, use_thumbnail=True, max_num=max_num)

    if aug:
        transform = _build_transform_aug(input_size=input_size)
        normalize = _build_normalize(input_size=input_size)            
        pixel_values = [transform(image=np.array(image.convert("RGB")))['image'] for image in images]    # use Albumentations for aug
        pixel_values = [normalize(image) for image in pixel_values]     # use Transform for normalize
    else:
