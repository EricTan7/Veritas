...
class CustomPreprocessor_Reason(MessagesPreprocessor):
    def preprocess(self, row: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        row = super().preprocess(row)
...
class CustomPreprocessor_Reason_RL(MessagesPreprocessor):
    def preprocess(self, row: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        row = super().preprocess(row)
        ...
        for i, item in enumerate(row["messages"]):
            if item["role"] == "assistant":
                row['solution'] = item["content"]
        return row
