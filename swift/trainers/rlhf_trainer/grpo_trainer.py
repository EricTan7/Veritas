...
class GRPOTrainer(RLHFTrainerMixin, SwiftMixin, HFGRPOTrainer):
    ...
    def __init__(self,
        ...
        self.loss_type = args.loss_type_grpo
        self.max_completion_length = args.max_completion_length
        self.epsilon_low = args.epsilon
        self.epsilon_high = args.epsilon_high if args.epsilon_high is not None else args.epsilon
        self.use_agrpo = args.use_agrpo
        self.agrpo_max, self.agrpo_min = args.agrpo_max, args.agrpo_min
    ...
    def _get_train_sampler(self):
        use_sequential = get_env_args('sequential_sampler', bool, False)
        if use_sequential:
        ...
        if "reward_type" in inputs[0]:
            for i, (reward_func, reward_template) in enumerate(zip(self.reward_funcs, self.reward_templates)):
                ...
                for idx in range(len(inputs)):
                    reward_for_cur_sample = inputs[idx]["reward_type"]
                    reward_idx_for_cur_sample = [self.name2idx[r] for r in reward_for_cur_sample]
                    if i in reward_idx_for_cur_sample:  # 当前样本用到了该reward
                        rewards_per_func[idx, i] = torch.tensor(output_reward_func[idx], dtype=torch.float32, device=device)
        else:
            for i, (reward_func, reward_template) in enumerate(zip(self.reward_funcs, self.reward_templates)):
                # reward model
    ...
    def _prepare_batch_inputs(self, inputs: InputsType, rewards: torch.Tensor) -> InputsType:
        ...
        # Corner case
        if self.use_agrpo:
            mean_val = rewards.view(-1, self.num_generations).mean(dim=1)
            max_val = rewards.view(-1, self.num_generations).max(dim=1)[0]
            thresh = 0.5
            advantages = advantages.reshape(-1, self.num_generations)
            advantages_mask = (mean_val == max_val) & (max_val < thresh)
            advantages_mask = advantages_mask.unsqueeze(1).expand(-1, self.num_generations)
            advantages = torch.where(advantages_mask, self.agrpo_min, advantages)
            advantages_mask = (mean_val == max_val) & (max_val > thresh)
            advantages_mask = advantages_mask.unsqueeze(1).expand(-1, self.num_generations)
            advantages = torch.where(advantages_mask, self.agrpo_max, advantages)
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if self.loss_type is None or self.loss_type == 'grpo':
