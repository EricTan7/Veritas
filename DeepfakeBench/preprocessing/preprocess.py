...
def extract_aligned_face_dlib(face_detector, predictor, image, res=256, mask=None):
    ...
    if len(faces):
        ...
        # cropped_face, mask_face = img_align_crop(rgb, landmarks, mask=mask)
...
def video_manipulate(
    ...
    def facecrop(
        ...
        if mode == 'fixed_num_frames':
            ...
            frame_idxs = np.linspace(10, frame_count_org - 1, num_frames, endpoint=True, dtype=int)
        elif mode == 'fixed_stride':
            # Get the frame rate of the video by dividing the number of frames by the duration (same interval between frames)
            frame_idxs = np.arange(0, frame_count_org, stride, dtype=int)
        # elif mode == "random_num_frames":
        #     frame_idxs = np.random.choice(np.arange(frame_count_org-1), num_frames * 3, replace=False)
...
if __name__ == '__main__':
   ...
   ## DeeperForensics-1.0
    ...
    # TalkingheadBench
    elif dataset_name == 'TalkingHeadBench':
        sub_dataset_names = [
            "fake/commercial_dataset",
            "fake/AniPortraitVideo/train", "fake/AniPortraitVideo/val", "fake/AniPortraitVideo/test", 
            "fake/EmoPortrait/train", "fake/EmoPortrait/val", "fake/EmoPortrait/test",
            "fake/Hallo/train", "fake/Hallo/val", "fake/Hallo/test",
            "fake/Hallo2/train", "fake/Hallo2/val", "fake/Hallo2/test",
            "fake/LivePortrait/train", "fake/LivePortrait/val", "fake/LivePortrait/test"
        ]
        sub_dataset_paths = [Path(os.path.join(dataset_path, name)) for name in sub_dataset_names]
    # TalkingheadBench
    elif dataset_name == 'hailuo_videos':
        sub_dataset_names = [
            ""
        ]
        sub_dataset_paths = [Path(os.path.join(dataset_path, name)) for name in sub_dataset_names]
