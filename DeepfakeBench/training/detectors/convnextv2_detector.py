...
@DETECTOR.register_module(module_name='convnextv2')
class ConvNextDetector(AbstractDetector):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.backbone = self.build_backbone(config)
        # fea_dim = self.backbone.encoder.stages[-1].layers[-1].pwconv2.weight.shape[0]
        fea_dim = self.backbone.timm_model.stages[-1].blocks[-1].mlp.fc2.weight.shape[0]
    ...
    def build_backbone(self, config):
        ...
        # model = ConvNextV2Model.from_pretrained(config['pretrained'])
        model = AutoModel.from_pretrained(config['pretrained'])
        # model.timm_model.head = nn.Identity()
        return model
