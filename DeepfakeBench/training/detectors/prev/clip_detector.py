...
@DETECTOR.register_module(module_name='clip_hf')
class CLIPHFDetector(AbstractDetector):
    def __init__(self, config):
        ...
        for name, param in self.backbone.named_parameters():
            if 'head.weight' not in name and 'head.bias' not in name:
                param.requires_grad_(False)
        
    def build_backbone(self, config):
        # prepare the backbone
        _, backbone = get_clip_visual(model_name='pretrain/clip/clip-vit-large-patch14')
        return backbone
